...
function createWebsiteCard(website) {
  ...
  card.addEventListener("click", () => {
      window.location.href = website.url; // Redirect to the specified URL
  });
...
function initialSetup() {
  ...
  // Add logout button to the floating menu
  const logoutButton = document.createElement('button');
  logoutButton.innerHTML = '<i class="fas fa-sign-out-alt"></i>';
  logoutButton.addEventListener('click', logout);
  document.querySelector('.floating-menu').appendChild(logoutButton);
}
// Select the elements
const menuToggle = document.getElementById('menu-toggle');
const navLinks = document.getElementById('nav-links');

// Toggle the class "show" on the nav links when the hamburger icon is clicked
